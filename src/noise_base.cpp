...
namespace godot {
    ...
    void NoiseBase::_bind_methods() {
        ...
        ClassDB::bind_method(
            D_METHOD("get_fbm_2d", "x", "y"),
            &NoiseBase::get_fbm_2d
        );
        ClassDB::bind_method(
            D_METHOD("get_fbm_3d", "x", "y", "z"),
            &NoiseBase::get_fbm_3d
        );
        ClassDB::bind_method(
            D_METHOD(
                "get_fbm_image_data",
    ...
    double NoiseBase::get_fbm_2d(double x, double y) const {
        double total = 0.0;
    ...
    double NoiseBase::get_fbm_3d(double x, double y, double z) const {
        double total = 0.0;
        double amplitude = 1.0;
        double frequency = 1.0;
        double max_value = 0.0;
...
#pragma omp parallel for
        ...
        for (int64_t y = 0; y < height; ++y) {
            for (int64_t x = 0; x < width; ++x) {
                double val = get_fbm_2d((x + offset_x) * scale, (y + offset_y) * scale);
                int64_t color = std::clamp(static_cast<int64_t>(
                    (val + 1.0) * 127.5),
    ...
    PackedByteArray NoiseBase::get_fbm_volume_data(
        ...
        ERR_FAIL_COND_V_MSG(width <= 0 || height <= 0 || depth <= 0, PackedByteArray(), "Dimensoes devem ser > 0.");
        constexpr int64_t MAX_BUFFER_BYTES = 512LL * 1024LL * 1024LL; // 512 MB Max
        ERR_FAIL_COND_V_MSG(width > (MAX_BUFFER_BYTES / height) / depth, PackedByteArray(), "Volume 3D excede 512MB de RAM.");
...
#pragma omp parallel for collapse(2)
        ...
        for (int64_t z = 0; z < depth; ++z) {
            for (int64_t y = 0; y < height; ++y) {
                for (int64_t x = 0; x < width; ++x) {
                    double val = get_fbm_3d((x + offset_x) * scale, (y + offset_y) * scale, (z + offset_z) * scale);
